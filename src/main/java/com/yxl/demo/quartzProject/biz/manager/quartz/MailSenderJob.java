...
import com.yxl.demo.quartzProject.biz.manager.MailSenderManager;
 ...
 * date: 2015-06-08 下午2:19
 */
public class MailSenderJob extends AbstarctSenderJob {

    private static final long serialVersionUID = -6L; //固定编号,以免自动生成quartz跟数据库对比产生不一致

    private static final Logger LOG = LoggerFactory.getLogger(MailSenderJob.class);

    //这里spring注入不进去,需要手动生成
//    protected static MailSenderManager mailSenderManager;
//
//    static {
//        mailSenderManager = (MailSenderManager) SpringBeanHelper.getBean("mailSenderManager");
//    }

    @Override
    protected String type() {
        return McSendChannelEnum.EMAIL.getCode();
    }

    @Override
    protected void doLogic() {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        System.out.println(sdf.format(new Date()) + "I'm enter mailSenderJob ");

        //1.根据类型获取 mc_msg 表里没有发送的邮件
//   FIXME TODO     McMsgDAO mcMsgDAO = (McMsgDAO) SpringBeanHelper.getBean("mcMsgDAO");
        MailSenderManager mailSenderManager = (MailSenderManager) SpringBeanHelper.getBean("mailSenderManager");
        ...
        for (McMsgDO msg : msgs) {
            try {
                ...
                EmailModelDTO dto = new EmailModelDTO();
                dto.setTo(msg.getReceivedUser());//收件人
                dto.setSubject(msg.getMsgTitle());//主题
                dto.setContent(msg.getMsgContent());//内容
                //3.发送
                boolean result = mailSenderManager.sendWithTemplate(dto, msg.toVelocityParamMap());
...
//  FIXME TODO    mcMsgDAO.updateMsg(msg);//update
                ...
                LOG.error("send email happend error ", e);
